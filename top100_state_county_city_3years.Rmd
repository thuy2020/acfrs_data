...
county_3years <- county_gov_20 %>% 
  ...
  #get a set of variables
  select(state.abb, state.name, id, geo_id, year, name,
         total_liabilities, net_pension_liability, net_opeb_liability, current_liabilities,
         net_pension_assets, net_opeb_assets, 
         expenses, revenues,
         population, urban_pop, pct_urban_pop, median_hh_income_21) %>% 
...
#TODO: Checking on 2 missing - as of April 12, 2024
# PA montgomery county 2022: https://www.montgomerycountypa.gov/Archive.aspx?AMID=45
# MA norfolk county 2022

#Uploaded April 12, 2024: 
# Ok Oklahoma county 2022: https://www.oklahomacounty.org/county-information/financials#Annual%20Financial%20Reports
#OK tulsa county 2022 https://countyclerk.tulsacounty.org/Home/Reports

top100_county_3years %>% 
  write.csv("output/top100_counties_3years.csv")

saveRDS(top100_county_3years, "top100_county_3years.RDS")
```

## Next 100
```{r}
top101_200_county_3years <- county_3years %>% 
  filter(geo_id %in% census_county_top101_200$geo_id) 

top101_200_county_3years%>% add_count(geo_id) %>% filter(n < 3) %>%
  select(state.abb, name, n) %>% distinct() 
...
special_cities <- acfrs_general_purpose_20 %>% 
  ...
  #get a set of variables
  select(state.abb, state.name, geo_id, id, year, name,
         total_liabilities, net_pension_liability, net_opeb_liability, current_liabilities,
         net_pension_assets, net_opeb_assets, 
         expenses, revenues, 
...
#missing
top100_cities_3years %>% add_count(geo_id) %>% filter(n<3) %>% arrange(name)
# missing: 3 as of April 12, 2024
# MN saint paul 2022
# NJ newark 2020, 
#NJ newark 2022

top100_cities_3years %>% filter(str_detect(name, "tulsa"))

top100_cities_3years %>% write.csv("output/top100_cities_3years.csv")

saveRDS(top100_cities_3years, "top100_cities_3years.RDS")
```

```{r}
top101_200_cities_3years <- city_gov_3years %>% filter(geo_id %in% census_city_top101_200$geo_id)  %>% distinct()

top101_200_cities_3years %>% add_count(geo_id) %>% select(state.abb, year, name, n) %>% 
  filter(n <3)

# check all these on portal. They are in the portal. Need to check the id to make sure they show up in the list top 200 here. 
