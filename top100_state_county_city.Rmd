...
# Resolve NA value for CA and SD 
#TODO: currently the acfrvalue in database is not fully populated. Fix it later --> don't have to impute 0 here. 
 #filter(is.na(net_pension_assets))
  mutate(net_pension_assets = ifelse(state.abb == "CA", 0, net_pension_assets),
         net_opeb_assets = ifelse(state.abb == "CA", 0, net_opeb_assets),
         current_assets = ifelse(state.abb == "CA" & year == 2020, 163031521000, current_assets),
         current_liabilities = ifelse(state.abb == "CA" & year == 2020, 103649199000, current_liabilities)
         ) %>% 
  
  mutate(current_assets = ifelse(state.abb == "DE"& year == 2020, 3812406000, current_assets),
         total_assets = ifelse(state.abb == "DE"& year == 2020, 14570384000, total_assets),
         current_assets = ifelse(state.abb == "DE"& year == 2021, 4944436000, current_assets),
         total_assets = ifelse(state.abb == "DE" & year == 2021, 16440516000, total_assets),
         net_pension_assets = ifelse(state.abb == "DE" & year == 2021, 8830000,
                                     net_pension_assets))
...
state_3years %>% filter(state.abb == "DE")
  select(state.abb, name, current_liabilities, current_assets) %>%
  filter(current_liabilities == 0)
...
top100_county_3years <- county_all %>% 
  ...
  #TODO: This is temporary fix. Will need to pull from acfrvalue table once the database is fixed.As of May 29, the current database does not allow to update. 
  
  mutate(total_assets = ifelse(name == "st louis county" & year == 2020, 2138938727, total_assets), 
         total_assets = ifelse(name == "st louis county" & year == 2021, 2199076860, total_assets),
         total_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2020, 403188793, total_assets),
         total_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2021, 476104005, total_assets),
         total_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2022, 509061826, total_assets),
         
         current_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2020, 403188793, current_assets),
         current_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2021, 476104005, current_assets),
          current_assets = ifelse(name == "essex county" & state.abb == "NJ" & year == 2022, 509061826, current_assets),
         
         net_pension_assets = ifelse(name == "oklahoma county" | name == "tulsa county", 0, net_pension_assets),
         net_opeb_assets = ifelse(name == "oklahoma county"| name == "tulsa county", 0, net_opeb_assets),
         current_assets = ifelse(name == "oklahoma county" & year == 2020, 146236643, current_assets), 
         net_opeb_liability = ifelse(name == "tulsa county" & year == 2022, 0, net_opeb_liability)) %>% 
  
  mutate(current_assets = ifelse(name == "san bernardino county" & state.abb == "CA" & year == 2021, 5287235000, current_assets),
         current_assets = ifelse(name == "san bernardino county" & state.abb == "CA" & year == 2020, 4809570000, current_assets))
...
top100_county_3years %>% select(state.abb, name, current_liabilities, current_assets) %>%
  filter(current_liabilities == 0)
  filter(if_any(everything(), ~. == 0))
...
top100_cities <- city_gov %>% 
  ...
  mutate(net_opeb_liability = ifelse(name == "jersey city" & state.abb == "NJ" & year == 2020, 1081996047, net_opeb_liability),
         total_assets = ifelse(name == "newark" & state.abb == "NJ" & year == 2021, 635368314, total_assets),
         current_assets = ifelse(name == "newark" & state.abb == "NJ" & year == 2021, 635368314, current_assets) )
...
top100_cities %>% filter(name == "newark")
```
