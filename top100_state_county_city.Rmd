...
top100_county_3years <- county_all %>% 
  ...
  #TODO: This is temporary fix. Will need to pull from acfrvalue table once the database is fixed.
  ...
  mutate(total_assets = ifelse(name == "st louis county" & year == 2020, 2138938727, total_assets), 
         ...
         total_assets = ifelse(name == "essex county", NA, total_assets),
...
top100_county_3years %>% 
  filter(name == "tulsa county") #%>% write.csv("tmp/essex_county.csv")
