...
# 
# state_3years %>% select(state.abb, id, name, year, current_assets) %>% arrange(name) %>% write.csv("tmp/current_assets_states_manual_input.csv")

state_3years %>% filter(state.abb == "NM")
```
...
top100_county_3years %>% filter(str_detect(name, "(?i)Pinellas County"))
top100_county_3years %>% filter(year == 2022)
...
# TODO: check why newark does not get the revenues as a sum of other columns
  mutate(revenues = ifelse(name == "newark" & state.abb == "NJ" & year == 2022, 943928678, revenues))
...
#test
top100_cities %>% filter(str_detect(name, "(?i)"))
```
