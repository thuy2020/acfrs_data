...
sd_db <- readRDS("data/acfrs_data.RDS") %>% 
  ...
  select(id, state.abb, name, ncesID) %>% 
  distinct()
...
acfrs_wo_ncesID <-  sd_db %>% filter(is.na(ncesID)) %>% select(id, state.abb, name) 
...
sd2020_namematched_Hgarb %>% 
  ...
  select(ncesID, state.abb, name) %>% filter(!is.na(ncesID)) %>% arrange(state.abb, name) %>% 
...
dictionary_3 <- t1 %>% left_join(t2, by = c("name", "state.abb")) %>% drop_na(id) %>% 
  rename(ncesID = ncesID.x) %>% 
  select(-ncesID.y) %>% arrange(state.abb, name)
...
dictionary123 <- dictionary1_2 %>% rbind(dictionary_3) %>% 
  ...
  )) %>% arrange(state.abb, name) %>% drop_na()

#########Dictionary 4##########
#Manual tracking in excel

# acfrs need ncesID
 sd_db %>% filter(!id %in% dictionary123$id) %>% select(state.abb, name, id, ncesID) %>%
  arrange(state.abb, name) #%>% write.csv("acfrs_need_add_ncesID.csv")

#hgarb leftover
sd2020_namematched_Hgarb %>% filter(!ncesID %in% dictionary123$ncesID) %>%
  drop_na(ncesID) %>% select(state.abb, name, ncesID, acfrs_original_name) %>%
  arrange(state.abb, name) #%>% write.csv("hgarb_leftover.csv")
...
nces_NOT_in_dictionary1234 <- nces %>% filter(!ncesID %in% dictionary1234$ncesID) %>% 
  select(state.abb, name_nces, ncesID) %>% 
  arrange(state.abb, name_nces) #%>% write.csv("nces_NOT_in_dictionary1234.csv")
