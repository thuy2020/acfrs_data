...
state_3years %>% #filter(is.na(net_pension_assets))
  mutate(net_pension_assets = ifelse(state.abb == "CA", 0, net_pension_assets),
         ...
         ) %>% write.csv("output/all_states_3years.csv")
saveRDS(state_3years, "state_3years.RDS") 


state_3years %>% select(name, compensated_absences, year) %>% arrange(name) %>% write.csv("tmp/state_compensated_absenses.csv")
  filter(compensated_absences == 0)
```
