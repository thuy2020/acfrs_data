...
PAcounty_census <- census_county %>% filter(state.abb == "PA") %>% 
  select(state.abb, name_census, population)

PA_counties <- county_gov %>% 
  filter(state.abb == "PA") %>% 
  select(all_of(fields_to_export)) %>% 
  arrange(desc(net_pension_liability)) %>% 
  mutate(category = "county") 
 
PA_counties %>% group_by(year) %>% 
  summarise(county_count = n()) %>% View()

#  population
  PA_counties %>%  filter(year == 2022) %>% View()
summarise(pop = sum(population, na.rm = TRUE)) %>% View()


#comparing census and acfrs collected
anti_join(PAcounty_census,
          acfrs_county %>% 
            filter(state.abb == "PA") %>% filter(year == 2022) %>% 
...
anti_join(acfrs_county %>% 
            filter(state.abb == "PA") %>% filter(year == 2021) %>% 
            select(state.abb, name),
...
PA_place_division_census <- census_place_division %>% filter(state.abb == "PA") %>% 
  select(state.abb, name_census, population) %>% 
  mutate(name_census = str_remove(name_census, "city$"),
         name_census = str_squish(name_census)) 

PA_place_division_census%>% 
  summarise(tot = sum(population, na.rm = TRUE)) 


PA_acfrs_municipality_22 <- place_division_gov %>% 
  filter(state.abb == "PA") %>% filter(year == 2022) %>% select(state.abb, name)

anti_join(
  PA_place_division_census, 
  PA_acfrs_municipality_22,
  by = c("name_census" = "name", "state.abb")
) %>% View()

# population covered in the current acfrs
PA_municipalities <- place_division_gov %>% 
  filter(state.abb == "PA") %>% #filter(year == 2022) %>% 
  select(all_of(fields_to_export)) %>% arrange(desc(net_pension_liability)) %>% 
...
total_collected <- data.frame(
  ...
  collected_population = c(6154920, 4961199, 3415263, 805579),
  total_count = c(1, 144, 1221, 517),
  collected_count = c(1, 61, 166, 314)
) %>% 
